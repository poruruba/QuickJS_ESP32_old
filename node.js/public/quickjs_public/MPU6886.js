...
export default class MPU6886 {
  ...
  readNByte(address, n) {
    ...
    this.wire.write(address);
    this.wire.endTransmission();
    this.wire.requestFrom(this._address, n);
  ...
  async Init() {
    ...
    this.imuId = this.readByte(IMU_6886_WHOAMI);
    console.log("imuId:" + this.imuId);
    await delay(1);

    regdata = 0x00;
    this.writeByte(IMU_6886_PWR_MGMT_1, regdata);
    delay(10);
    ...
    // +- 8g
    regdata = 0x10;
    this.writeByte(IMU_6886_ACCEL_CONFIG, regdata);
    delay(1);

    // +- 2000 dps
    regdata = 0x18;
    this.writeByte(IMU_6886_GYRO_CONFIG, regdata);
    delay(1);

    // 1khz output
    regdata = 0x01;
    this.writeByte(IMU_6886_CONFIG, regdata);
    delay(1);

    // 2 div, FIFO 500hz out
    regdata = 0x01;
    this.writeByte(IMU_6886_SMPLRT_DIV, regdata);
    delay(1);

    regdata = 0x00;
    this.writeByte(IMU_6886_INT_ENABLE, regdata);
    delay(1);

    regdata = 0x00;
    this.writeByte(IMU_6886_ACCEL_CONFIG2, regdata);
    delay(1);

    regdata = 0x00;
    this.writeByte(IMU_6886_USER_CTRL, regdata);
    delay(1);

    regdata = 0x00;
    this.writeByte(IMU_6886_FIFO_EN, regdata);
    delay(1);

    regdata = 0x22;
    this.writeByte(IMU_6886_INT_PIN_CFG, regdata);
    delay(1);
  ...
  getAccelAdc() {
    var buf = this.readNByte(IMU_6886_ACCEL_XOUT_H, 6);
  ...
  getGyroAdc() {
    var buf = this.readNByte(IMU_6886_GYRO_XOUT_H, 6);
  ...
  getTempAdc() {
    var buf = this.readNByte(IMU_6886_TEMP_OUT_H, 2);
  ...
  getTempData() {
    var temp = this.getTempAdc();

    return temp / 326.8 + 25.0;
  }
}

function comp2(val, num_of_bytes){
	var mask = 0x01;
