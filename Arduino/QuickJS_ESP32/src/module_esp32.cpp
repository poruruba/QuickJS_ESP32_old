...
static JSValue esp32_reload(JSContext *ctx, JSValueConst jsThis, int argc, JSValueConst *argv)
{
  const char *url = NULL;
  if( argc >= 1 )
...
static JSValue esp32_enable_logio(JSContext *ctx, JSValueConst jsThis, int argc,
                            JSValueConst *argv) {
  enable_logio = JS_ToBool(ctx, argv[0]);
  return JS_UNDEFINED;
}
...
static const JSCFunctionListEntry esp32_funcs[] = {
    ...
    JSCFunctionListEntry{"reload", 0, JS_DEF_CFUNC, 0, {
                           func : {1, JS_CFUNC_generic, esp32_reload}
                         }},
